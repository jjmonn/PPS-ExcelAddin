...
#Region "Instance Variables"
    ...
    Friend m_associatedWorksheetName As String = ""

    ' Variables
    Private m_disableWSChangeFlag As Boolean
    '  Friend m_reportUploadController.AssociatedWorksheet As Excel.Worksheet
...
#Region "Initialize"
    ...
    Friend Sub AssociateSubmissionWSController(ByRef p_reportUploadController As ReportUploadControler)

        m_reportUploadController = p_reportUploadController
        m_logView = New LogView(True)

        Select Case m_reportUploadController.DataSet.m_processFlag
            Case Account.AccountProcess.RH
                Dim version As Version = GlobalVariables.Versions.GetValue(m_reportUploadController.DataSet.m_currentVersionId)
        ...
        m_associatedWorksheetName = p_reportUploadController.AssociatedWorksheet.Name
...
#Region "Update Worksheet"
    ...
    Friend Sub UpdateFinancialCalculatedItemsOnWS(ByRef p_entitiesName() As String)
        ...
        For Each l_entityName As String In p_entitiesName
            ...
            If Not l_entity Is Nothing Then
                For Each l_account As Account In m_reportUploadController.AcquisitionModel.m_outputsList
                    For Each l_period As Int32 In m_reportUploadController.AcquisitionModel.m_currentPeriodList
                        If Not l_account Is Nothing Then
                            If l_account.FormulaType = Account.FormulaTypes.FIRST_PERIOD_INPUT Then
                                If l_period <> m_reportUploadController.AcquisitionModel.m_currentPeriodList(0) Then
    ...
    Private Sub SetDatsetCellValue(ByRef p_entityId As Int32, _
        ...
        If m_reportUploadController.Dataset.m_datasetCellsDictionary.ContainsKey(tuple_) = True Then
            Dim l_account = GlobalVariables.Accounts.GetValue(p_accountName)

            If l_account Is Nothing Then Exit Sub
            Dim value = m_reportUploadController.AcquisitionModel.GetCalculatedValue(p_entityId, _
                                                            l_account.Id, _
                                                            m_reportUploadController.AcquisitionModel.m_periodsIdentifyer & p_period)
            If Double.IsNaN(value) Then value = 0

            m_reportUploadController.Dataset.m_datasetCellsDictionary(tuple_).Value2 = value
        End If
    ...
    Friend Sub UpdateFinancialInputsOnWorsheet()
        ...
        For Each l_entityName As String In m_reportUploadController.Dataset.m_dimensionsAddressValueDict(ModelDataSet.Dimension.ENTITY).Values
            For Each l_account As Account In m_reportUploadController.Dataset.m_inputsAccountsList
                For Each l_period As Int32 In m_reportUploadController.AcquisitionModel.m_currentPeriodList
                    If m_reportUploadController.AcquisitionModel.m_databaseInputsDictionary(l_entityName)(l_account.Name).ContainsKey(m_reportUploadController.AcquisitionModel.m_periodsIdentifyer & l_period) = True Then
                        l_value = m_reportUploadController.AcquisitionModel.m_databaseInputsDictionary(l_entityName)(l_account.Name)(m_reportUploadController.AcquisitionModel.m_periodsIdentifyer & l_period)
                    Else
                        l_value = 0
                    End If
                    ...
                    m_reportUploadController.Dataset.m_datasetCellsDictionary(l_tuple).Value2 = l_value
                Next
            Next
        Next
    ...
    Friend Sub UpdateRHInputsOnWorsheet()
        ...
        Dim l_entityName As String = m_reportUploadController.Dataset.m_dimensionsAddressValueDict(ModelDataSet.Dimension.ENTITY).Values(0)
        Dim l_accountName As String = m_reportUploadController.Dataset.m_dimensionsAddressValueDict(ModelDataSet.Dimension.ACCOUNT).Values(0)

        For Each l_employeeName As String In m_reportUploadController.Dataset.m_dimensionsAddressValueDict(ModelDataSet.Dimension.EMPLOYEE).Values
            For Each l_period As Int32 In m_reportUploadController.Dataset.m_dimensionsValueAddressDict(ModelDataSet.Dimension.PERIOD).Keys

                Dim l_fact As CRUD.Fact = m_reportUploadController.FactsStorage.GetRHFact(l_accountName, l_employeeName, m_periodIdentifier & l_period)
...
#Region "Worksheet change"
    ...
    Private Sub WorksheetChangePDC(ByVal p_target As Excel.Range)
        ...
        If m_reportUploadController.m_isUpdating = False AndAlso m_disableWSChangeFlag = False Then
            ...
            For Each l_cell As Excel.Range In p_target.Cells
                ...
                Dim l_intersect = GlobalVariables.APPS.Intersect(l_cell, m_reportUploadController.datamodificationtracker.m_dataSetRegion)
                If Not l_intersect Is Nothing Then

                    If VarType(l_cell.Value) = VariantType.String _
                    Or l_cell.Value2 Is Nothing Then
                    ...
                    Else
                        ...
                        m_disableWSChangeFlag = True
                        l_cell.Value = m_reportUploadController.DataSet.m_datasetCellDimensionsDictionary(l_cell.Address).m_client
                        m_disableWSChangeFlag = False
                    End If
                End If
            Next
    ...
    Private Sub RHCellsModificationTreatment(ByRef p_cell As Excel.Range, _
        ...
        SyncLock (m_reportUploadController)
            Dim l_cellDimensions = m_reportUploadController.DataSet.m_datasetCellDimensionsDictionary(p_cell.Address)
            ...
            If Not l_dependantCells Is Nothing Then
                For Each l_dependantCell In l_dependantCells.Cells
                    Dim l_dependantIntersect = GlobalVariables.APPS.Intersect(l_dependantCell, m_reportUploadController.datamodificationtracker.m_dataSetRegion)
...
#End Region
    ...
    Private Sub DisplayLogButton_Click(ctrl As CommandBarButton, ByRef cancelDefault As Boolean)
        ...
        If m_reportUploadController.DataSet.m_datasetCellDimensionsDictionary.ContainsKey(m_currentCellAddress) Then
            Dim datasetCellStruct As ModelDataSet.DataSetCellDimensions = m_reportUploadController.DataSet.m_datasetCellDimensionsDictionary(m_currentCellAddress)
    ...
    Friend Sub Worksheet_SelectionChange(ByVal Target As Excel.Range)
        ...
        If m_reportUploadController.DataSet.m_datasetCellDimensionsDictionary.ContainsKey(Target.Address) Then
            Dim datasetCellStruct As ModelDataSet.DataSetCellDimensions = m_reportUploadController.DataSet.m_datasetCellDimensionsDictionary(Target.Address)
        ...
        ElseIf m_reportUploadController.DataSet.m_dimensionsAddressValueDict(ModelDataSet.Dimension.ACCOUNT).ContainsKey(address) Then
            GlobalVariables.s_reportUploadSidePane.DisplayAccountDetails(GlobalVariables.Accounts.GetValueId(m_reportUploadController.DataSet.m_dimensionsAddressValueDict(ModelDataSet.Dimension.ACCOUNT)(address)))
        ElseIf m_reportUploadController.DataSet.m_dimensionsAddressValueDict(ModelDataSet.Dimension.OUTPUTACCOUNT).ContainsKey(address) Then
            GlobalVariables.s_reportUploadSidePane.DisplayAccountDetails(GlobalVariables.Accounts.GetValueId(m_reportUploadController.DataSet.m_dimensionsAddressValueDict(ModelDataSet.Dimension.OUTPUTACCOUNT)(address)))
        Else
...
#Region "Utilities"
    ...
    Private Function CellBelongsToFinancialDimensionsDefinition(ByVal p_address As String) As Boolean
        ...
        If m_reportUploadController.Dataset.m_dimensionsAddressValueDict(ModelDataSet.Dimension.ACCOUNT).ContainsKey(p_address) Then
            m_disableWSChangeFlag = True
            m_reportUploadController.AssociatedWorksheet.Range(p_address).Value = m_reportUploadController.Dataset.m_dimensionsAddressValueDict(ModelDataSet.Dimension.ACCOUNT)(p_address)
            m_disableWSChangeFlag = False
            Return True
        ElseIf m_reportUploadController.Dataset.m_dimensionsAddressValueDict(ModelDataSet.Dimension.PERIOD).ContainsKey(p_address) Then
            m_disableWSChangeFlag = True
            m_reportUploadController.AssociatedWorksheet.Range(p_address).Value = m_reportUploadController.Dataset.m_dimensionsAddressValueDict(ModelDataSet.Dimension.PERIOD)(p_address)
            m_disableWSChangeFlag = False
            Return True
        ElseIf m_reportUploadController.Dataset.m_dimensionsAddressValueDict(ModelDataSet.Dimension.ENTITY).ContainsKey(p_address) Then
            m_disableWSChangeFlag = True
            m_reportUploadController.AssociatedWorksheet.Range(p_address).Value = m_reportUploadController.Dataset.m_dimensionsAddressValueDict(ModelDataSet.Dimension.ENTITY)(p_address)
            m_disableWSChangeFlag = False
            Return True
        End If
        Return False
    ...
    Private Function CellBelongsToPDCDimensionsDefinition(ByVal p_address As String) As Boolean
        ...
        p_address = Replace(p_address, "$", "")
        If m_reportUploadController.Dataset.m_dimensionsAddressValueDict(ModelDataSet.Dimension.EMPLOYEE).ContainsKey(p_address) Then
            m_disableWSChangeFlag = True
            m_reportUploadController.AssociatedWorksheet.Range(p_address).Value = m_reportUploadController.Dataset.m_dimensionsAddressValueDict(ModelDataSet.Dimension.EMPLOYEE)(p_address)
            m_disableWSChangeFlag = False
            Return True
        ElseIf m_reportUploadController.Dataset.m_dimensionsAddressValueDict(ModelDataSet.Dimension.PERIOD).ContainsKey(p_address) Then
            m_disableWSChangeFlag = True
        ...
        ElseIf m_reportUploadController.Dataset.m_dimensionsAddressValueDict(ModelDataSet.Dimension.ENTITY).ContainsKey(p_address) Then
            ...
            m_disableWSChangeFlag = False
            Return True
        ElseIf m_reportUploadController.Dataset.m_dimensionsAddressValueDict(ModelDataSet.Dimension.ACCOUNT).ContainsKey(p_address) Then
            m_disableWSChangeFlag = True
            m_reportUploadController.AssociatedWorksheet.Range(p_address).Value = m_reportUploadController.Dataset.m_dimensionsAddressValueDict(ModelDataSet.Dimension.ACCOUNT)(p_address)
            m_disableWSChangeFlag = False
            Return True
    ...
    Private Sub RHRegisterModificationIfDifferentValue(ByRef p_cell As Excel.Range, _
        ...
        Dim l_fact = m_reportUploadController.FactsStorage.GetRHFact(p_cellDimensions.m_accountName, _
                                              p_cellDimensions.m_employee, _
                                              m_periodIdentifier & p_cellDimensions.m_period)
        If l_fact Is Nothing Then
            If p_cell.Value2 Is Nothing Then Exit Sub
        ...
        If p_cell.Value2 Is Nothing Then
            p_updateFlag = True
            m_reportUploadController.RegisterModification(p_cell.Address)
        End If

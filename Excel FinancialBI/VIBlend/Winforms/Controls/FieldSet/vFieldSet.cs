...
// Type: VIBlend.WinForms.Controls.vFieldSet
...
{
  /// <summary>
  /// Represents a vFieldSet control that displays and drop-down list of multi-choice items.
  /// </summary>
  [Designer("VIBlend.WinForms.Controls.Design.vFieldSetDesignerBase, VIBlend.WinForms.Controls.Design, Version=10.1.0.0, Culture=neutral, PublicKeyToken=6feea38fef5ea0cf")]
  [ToolboxBitmap(typeof (vFieldSet), "ControlIcons.vFieldSet.ico")]
  [Description("Displays a drop-down list of multi-choice items.")]
  [ToolboxItem(true)]
  public class vFieldSet : vCheckBox
  {
    private vPanel container = new vPanel();
    private List<vCheckBox> checkboxes = new List<vCheckBox>();
    ...
    /// <summary>
    /// Gets a reference to the DropDown window of the vFieldSet control
    /// </summary>
    ...
    /// <summary>
    /// Gets or sets the the horizontal spacing for the FieldSet control.
    /// </summary>
    [Category("Behavior")]
    ...
    /// <summary>
    /// Gets or sets the the vertical spacing for the FieldSet control.
    /// </summary>
    [Category("Behavior")]
    [Description("Gets or sets the vertical spacing.")]
    ...
    /// <summary>
    /// Gets a reference to the CheckBoxes list of the FieldSet control
    /// </summary>
    [Description("Gets the check boxes.")]
    [Category("Behavior")]
    ...
    /// <summary>
    /// Gets or sets the container panel of the FieldSet control
    /// </summary>
    [Browsable(false)]
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="T:VIBlend.WinForms.Controls.vFieldSet" /> class.
    /// </summary>
    public vFieldSet()
    {
    ...
    private void vFieldSet_PropertyChanged(object sender, PropertyChangedEventArgs e)
    {
      if (!(e.PropertyName == "Theme"))
        return;
      foreach (vCheckBox checkBox in this.CheckBoxes)
        checkBox.VIBlendTheme = this.VIBlendTheme;
    ...
    private void HandleCheckedChanged()
    {
      ...
      if (!this.Checked)
      {
        ...
        vFieldSet.ShowWindow(new HandleRef((object) this.dropDown, this.dropDown.Handle), 8);
      }
      else
        vFieldSet.ShowWindow(new HandleRef((object) this.dropDown, this.dropDown.Handle), 8);
      this.Animate();
    ...
    private void CalculateBounds()
    {
      ...
      foreach (vCheckBox checkBox in this.CheckBoxes)
      {
        this.Panel.Content.Controls.Add((Control) checkBox);
        checkBox.Location = new Point(this.hSpacing, verticalSpacing);
        verticalSpacing += checkBox.Height + this.VerticalSpacing;
        val1 = Math.Max(val1, checkBox.Size.Width);
        checkBox.Theme = this.Panel.Theme;
      }

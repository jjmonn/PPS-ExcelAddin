...
// Type: VIBlend.WinForms.Controls.vPercentageEditor
...
namespace VIBlend.WinForms.Controls
{
  ...
  [ToolboxBitmap(typeof (vFixedPointEditor), "ControlIcons.vPercentageEditor.ico")]
  [Description("Provides an editable text field with cutomizable features.")]
  public class vPercentageEditor : InputBase
  {
    ...
    public override string SeparatorChar
    {
      get
      {
        return StringUtility.GetPercentageSeparator(this.CultureInfo);
      }
    }

    /// <summary>Represents a new instance of PercentageEditor.</summary>
    public vPercentageEditor()
    {
      this.baseFormatString = "p";
    ...
    protected override bool SetFormattedText()
    {
      try
      {
        ...
        Decimal num1 = Decimal.Parse(this.valueString, (IFormatProvider) CultureInfo.InvariantCulture) / new Decimal(100);
    ...
    {
      if (this.SelectionStart == this.Text.Length && this.DecimalPlaces > 0 || this.SelectionStart > 2 && this.CultureInfo.NumberFormat.PercentSymbol.ToString().Equals(this.Text[this.SelectionStart].ToString()))
        return;
      if ((int) this.Text[this.SelectionStart] == 32)
      {
        int selectionStart = this.SelectionStart;
        if (selectionStart + 1 <= this.Text.Length - 1 && this.Text[selectionStart + 1].ToString().Equals(this.CultureInfo.NumberFormat.PercentSymbol.ToString()))
          return;
      }
      if (insertion.Equals("-"))
      {
      ...
      else
      {
        ...
        else
        {
          ...
          if (flag)
            --selectionInValue;
          this.SelectionStart = StringUtility.GetSelectionPositionFromValuePosition(selectionInValue + 1, this.Text, this.SeparatorChar, this.HasSeparator);
    ...
    {
      vPercentageEditor percentageEditor;
      int num;
      for (; this.Text.Length - this.SelectionStart != position && this.SelectionStart + 1 <= this.Text.Length; percentageEditor.SelectionStart = num)
      {
        percentageEditor = this;
        num = percentageEditor.SelectionStart + 1;
      }
    }
    ...
    private void Delete(bool deleteWithoutSelection)
    {
      ...
      {
        this.OnValueChanged(new ValueChangedEditorEventArgs(this.Value));
        this.SelectionStart = StringUtility.GetSelectionPositionFromValuePosition(selectionInValue, this.Text, this.SeparatorChar, this.HasSeparator);

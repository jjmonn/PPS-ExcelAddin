...
// Type: VIBlend.WinForms.Controls.vNumberEditor
...
namespace VIBlend.WinForms.Controls
{
  ...
  [ToolboxBitmap(typeof (vFixedPointEditor), "ControlIcons.vNumberEditor.ico")]
  public class vNumberEditor : InputBase
  {
    ...
    /// <summary>Initializes a new instance of NumberEditor.</summary>
    public vNumberEditor()
    {
      this.baseFormatString = "N";
    ...
    public override void Insert(string insertion)
    {
      ...
      else
      {
        ...
        if (this.DecimalPlaces > 0 && selectionInValue >= this.ValueString.Length)
          --selectionInValue;
        if (this.DecimalPlaces > 0 && selectionInValue < this.ValueString.Length && (int) this.ValueString[selectionInValue] == 46)
        {
    ...
    {
      vNumberEditor vNumberEditor;
      int num;
      for (; this.Text.Length - this.SelectionStart != position && this.SelectionStart + 1 <= this.Text.Length; vNumberEditor.SelectionStart = num)
      {
        vNumberEditor = this;
        num = vNumberEditor.SelectionStart + 1;
      }
    }
    ...
    private void Delete(bool deleteWithoutSelection)
    {
      ...
      else
      {
        ...
        this.SelectionStart = fromValuePosition;
        if (this.DecimalPlaces <= 0 || fromValuePosition < this.valueString.IndexOf('.') || this.isBackSpace)
          return;
        ++this.SelectionStart;
      }
    }

...
namespace FBI.MVC.View
{
  ...
  public partial class AllocationKeysView : Form, IView
  {
    ...
    private void OnModelRead(ErrorMessage p_status, EntityDistribution p_attributes)
    {
      ...
      else
      {
        if (p_status == ErrorMessage.SUCCESS)
        {
          ...
          this.m_allocationsKeysDGV.FillField<double, TextBoxEditor>(p_attributes.EntityId, 42, p_attributes.Percentage, new TextBoxEditor());
        }
      }
    }
    ...
    {
      this.m_allocationsKeysDGV.InitializeRows<AxisElem>(AxisElemModel.Instance, AxisElemModel.Instance.GetDictionary(AxisType.Entities));

      MultiIndexDictionary<UInt32, string, AxisElem> l_axisElemMID = AxisElemModel.Instance.GetDictionary(AxisType.Entities);

      this.m_allocationsKeysDGV.SetDimension(FbiDataGridView.Dimension.COLUMN, 42, "Percentage %");
      foreach (AxisElem l_entity in l_axisElemMID.SortedValues)
      {
        ...
        if (l_entityDistrib == null)
          this.m_allocationsKeysDGV.FillField<double, TextBoxEditor>(l_entity.Id, 42, 0.0, this.CreateTextBoxEditor(l_entity.AllowEdition));
        else
          this.m_allocationsKeysDGV.FillField<double, TextBoxEditor>(l_entity.Id, 42, l_entityDistrib.Percentage, this.CreateTextBoxEditor(l_entity.AllowEdition));
      }
    ...
    private void ChangeParentPercentage(UInt32 p_parentId)
    {
      ...
      this.m_allocationsKeysDGV.FillField<double, TextBoxEditor>(p_parentId, 42, l_percentage, this.CreateTextBoxEditor(false));
    }

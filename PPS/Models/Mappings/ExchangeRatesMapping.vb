...
Public Class ExchangeRatesMapping
    ...
    Public Shared Sub FillRatesLists(ByVal currency_token As String, _
        ...
        srv.openRstSQL("SELECT * FROM " & CONFIG_DATABASE & "." & EXCHANGE_RATES_TABLE_NAME & _
                       " WHERE " & EX_TABLE_CURRENCY_VARIABLE & "='" & currency_token & "'" & " AND " & _
                         EX_TABLE_RATE_VERSION & "='" & rates_version & "'", _
        ...
        If srv.rst.EOF = False And srv.rst.BOF = False Then
            If time_config = YEARLY_TIME_CONFIGURATION Then
                srv.rst.MoveFirst()
                Do While srv.rst.EOF = False
                    periods_list.Add(srv.rst.Fields(EX_TABLE_PERIOD_VARIABLE).Value)
                    If reverse_flag = 1 Then
                        rates.Add(1 / srv.rst.Fields(EX_TABLE_RATE_VARIABLE).Value)
                    Else
                        rates.Add(srv.rst.Fields(EX_TABLE_RATE_VARIABLE).Value)
                    End If
                    srv.rst.MoveNext()
                Loop
            Else
                Dim months_list As List(Of Integer) = Periods.GetMonthlyPeriodsList(ref_period, 1)
                For Each period In months_list
                    periods_list.Add(period)
                    srv.rst.Filter = EX_TABLE_PERIOD_VARIABLE & "=" & period
                    If srv.rst.EOF = False And srv.rst.BOF = False Then
                        If reverse_flag = 1 Then
                            rates.Add(1 / srv.rst.Fields(EX_TABLE_RATE_VARIABLE).Value)
                        Else
